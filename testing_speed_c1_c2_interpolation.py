...
if __name__ == '__main__':
    ...
    betas = [0.4,0.45,0.5,0.55,0.6]#-1000000, 0.2,0.25,0.3,0.35,0.4,
    ...
    pickle.dump(speeds_alpha_20,open( './data/speeds_alpha_20_40to60.pkl','wb'))
    pickle.dump(speeds_std_alpha_70, open('./data/speeds_alpha_70_40to60.pkl', 'wb'))
    pickle.dump(speeds_alpha_90, open('./data/speeds_alpha_90_40to60.pkl', 'wb'))


    pickle.dump(speeds_std_alpha_20,open( './data/speeds_std_alpha_20_40to60.pkl','wb'))
    pickle.dump(speeds_std_alpha_70, open('./data/speeds_std_alpha_70_40to60.pkl', 'wb'))
    pickle.dump(speeds_std_alpha_90, open('./data/speeds_std_alpha_90_40to60.pkl', 'wb'))

    plt.figure(1)

    plt.errorbar(network_size**np.asarray(betas), speeds_alpha_20, yerr=speeds_std_alpha_20, color='r', linewidth=2.5,
                 label='$q_n = '+str(Decimal(1 / network_size ** alpha_1).quantize(FOURPLACES))+'$')
    #
    # plt.errorbar(network_size**np.asarray(betas), speeds_alpha_50, yerr=speeds_std_alpha_50, color='g', linewidth=2.5,
    #              label='$\\alpha = 0.5$')

    # plt.errorbar(network_size ** np.asarray(betas), speeds_alpha_60, yerr=speeds_std_alpha_60, color='c', linewidth=2.5,
    #              label='$\\alpha = 0.6$')

    plt.errorbar(network_size ** np.asarray(betas), speeds_alpha_70, yerr=speeds_std_alpha_70, color='g', linewidth=2.5,
                 label='$q_n = '+str(Decimal(1 / network_size ** alpha_2).quantize(FOURPLACES))+'$')

    plt.errorbar(network_size**np.asarray(betas), speeds_alpha_90, yerr=speeds_std_alpha_90, color='b', linewidth=2.5,
